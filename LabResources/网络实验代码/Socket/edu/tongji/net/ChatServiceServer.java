...
public class ChatServiceServer extends Thread{//implements Runnable {
	...
	PrintWriter out;
	public ChatServiceServer(Socket c)throws IOException{
		this.client=c;
	...
	private static void Response(String message){
		...
		while(it.hasNext()){
			ChatServiceServer aServer=(ChatServiceServer)it.next();
			aServer.sendResponse(message);
		}
	}
	public static void setUserList(Collection userList) {
		ChatServiceServer.userList = userList;
	}
